...
% the list "pairs" will be the first work packet to be run.
    for each p in pairs do work_queue_insert p;
% I keep a record of which pairs are currently being processed.
    active_pairs := nil;

% This sort of shows how I want to farm to behave. Keep going so long
% as there is more work to be done.
    while not (p := get_from_work_queue()) do begin
% track which pair I am working on. I probably want this opration to be
% part of an atomic action along with the act of grabbing a pair from the
% queue.
      active_pairs := p;
% Now I have the work that I perform for each task. Create an s-polynomial.
      scalar s := s_poly(car p, cdr p);
...
% then this work packet has completed.
      if null s then <<
% If a pair completes but does not add any new information then I just need
% to remove it from the record of active work. This update needs to be
% made atomic I expect!
         active_pairs := delete(p, active_pairs);
...
% should let me end up with a miminal basis.
        ...
        L := s . L;
        active_pairs := delete(p, active_pairs);
% Any time I make progress by updating the current base I will
% arrange that each pair currently being processed is queued to be processed
% again. Now I rather expect that handling a pair for a second time will
% often have no effect and it may even be fast.
        for each op in active_pairs do
           reinstate_on_work_queue op;
        active_pairs := nil
       end
    end;
    terpri();
