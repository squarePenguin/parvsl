...
symbolic procedure unpredictable();
  begin
    scalar n := 0;
    for each x in explodecn date!-and!-time() do
       n := remainder(19937*n + x, 3141592653);
    princ "Random value = ";
    print n;
    return n
  end;
...
random_new_seed (starting_seed := unpredictable());
...
begin
    scalar p, s, pairs, L, L1, numdone, done;
...
algebraic babygroe {x*y-x, x^2-y};

algebraic babygroe {x^3 - 2*x*y,
                    x^2*y - 2*y^2 + x};
...
% When I restart VSL I can pass data from the old to the new copy. Here
% I use this so that each time I run a test it is in a fresh new Reduce.
% I will try my test with from 1 to 8 threads and then do that again
% for 4 random orderings of the pairs tthat get treated.

symbolic procedure restartable nn;
begin
  scalar thr, seed;
  thr := remainder(nn, 32);
  seed = nn/32;
...
begin
  ...
  wt0 := walltime();
  t0 := time();    
% Here to provide more scope for parallelism (albeit in a cheating way!)
% I replicate the first polynomial in my input so that if there are N copies
...
% in the heap-image that I reload...

lisp preserve('begin, "groeREDUCE", t);
