...
list("lispsystem!*, !*savedef = ", lispsystem!*, !*savedef);
...
!#if (or !*savedef (memq 'embedded lispsystem!*))
    ...
    else restart!-csl('(remake1 build_reduce_modules), names)
!#else
...
% This illustrates that I need a proper general formatted print mechanism!
    ...
    if null names then <<
        ...
        restart!-csl t >>
    else restart!-csl('(remake1 test_a_package), names)
  end;
...
% input & output file selection is not messed up for me.
    ...
    if null names then <<
        ...
        restart!-csl t >>
    else restart!-csl('(remake1 profile_a_package), names)
  end;
...
(cond
   ((memq 'vsl lispsystem!*) (rdf "$srcdir/parvsl.lsp")))
