...
% from C++, however we can take the address of a stack variable and hence approximate the
...
% scans all locations in between for ambiguous roots.
...
% The alternative, a \emph{real-time} garbage collector would require
% a major redesign using additional interlocks for data access. Implementing one efficiently probably merits its
% own project. For the small thread counts of current day computers I believe stop-the-world to be
% adequate (the main drawback of stop-the-world is not overall speed but latency, which I am less concerned with),
% although this may change as core counts increase.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\linewidth]{gcdeadlock.pdf}
...
% TODO: repeats section with locks. just refer to it
