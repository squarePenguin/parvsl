...
to handle single character reads and writes. Only flush operation is synchronised with the file mutex when it
flushes these buffers (or fills them in the case of reading).

Finally there is the issue shared data in Lisp. The user can cause data races by modifying the same global symbol
from multiple threads, or using other side-effectful features such as modifying a list in place. I could lock a
mutex for any such operation to ensure safety, however such a change would add a massive overhead to ParVSL.
I have decided to leave this issue as a responsibility to the ParVSL user. I discuss in section \ref{sec:threads}
how I expose mutex locks in ParVSL. My choice means that the user can optimise their code to minimise
lock usage, at the expense of safety.
