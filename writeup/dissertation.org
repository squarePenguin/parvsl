...
#+BEGIN_SRC C++
{
  Foo obj1; // obj1 is constructed here, unlike Java, where it would be a null reference 
  Bar obj2;
} // End of scope, obj2 is destructed,
...
smart pointer objects become inaccessible (i.e by going out of scope), the underlying pointer
...
** Garbage collection
I will use the shortened form GC to refer to garbage collection.
...
of memory to managed by incrementing and derementing a stack pointer. The stack pointer is normally not accessible
...
int main() {
  int t; // t is any variable on the stack, before execution of VSL code begins

  // When starting VSL, we store the base of the stack
  // Here we need to cast the reference to an in type then align to the size of a LispObject
...
void garbage_collection() {
  ...
  for (uintptr_t s = C_stackhead; s < C_stackbase; s += sizeof(LispObject)) {
