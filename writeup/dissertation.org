...
is just about the only working example. The interpreter is non thread-safe and all contention
on global variables (including printing to the same stream) leads to undefined behaviour. Printing
...
** Computing Groebner basis in parallel
